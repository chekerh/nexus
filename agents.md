...
- Use ECC's `tdd-workflow` for new behavior and bug fixes when practical.
- Use ECC's `security-review` for authentication, billing, publishing, uploads, and token-handling changes.
- Use ECC's `verification-loop` before shipping changes to run focused tests, checks, and security validation.
